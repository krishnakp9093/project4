...
public class LoginCtl extends BaseCtl {
	...
	protected boolean validate(HttpServletRequest request) {
		...
		if (DataValidator.isNull(login)) {
			System.out.println("loginctl isNull (in login) run hui");
			request.setAttribute("login", PropertyReader.getValue("error.require", "Login Id"));
			pass = false;
		} else if (!DataValidator.isEmail(login)) {
			System.out.println("loginctl isEmail");
			request.setAttribute("login", PropertyReader.getValue("error.email", "Login Id"));
			pass = false;
		}
		if (DataValidator.isNull(request.getParameter("password"))) {
			System.out.println("loginctl isNull (in password) run hui");
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (OP_LOG_OUT.equals(op) && !OP_SIGN_IN.equals(op)) {
			System.out.println("operation " + op + "  mila");
		...
		System.out.println("DoGet se view pr forward kiya");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
			...
			try {
				...
				if (bean != null) {
					...
					if ("null".equalsIgnoreCase(uri)) {
						...
						System.out.println("null uri vali chli");
						return;
					} else {
						ServletUtility.redirect(uri, request, response);
						System.out.println("else uri redict chli");

						return;
					}

				} else {
					System.out.println("Else run hui");
		...
		{
			System.out.println("operation " + op + "  mila");
			System.out.println("LoginCtl Do post se UserRegistrationCtl se redirect kiya");
